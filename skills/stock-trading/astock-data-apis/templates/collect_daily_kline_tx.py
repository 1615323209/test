#!/usr/bin/env python3
"""A股15年日K批量采集 — 腾讯源 后复权(hfq) 模板
不受云服务器IP限制。过滤：排除ST/北交所/上市<250天。
断点续传，Parquet 输出。适配路径后直接运行。

⚠️ 必须用 hfq（后复权）——腾讯源前复权(qfq)对高分红股票产生负数价格。
"""
...
START_DATE = "20100101"
ADJUST = "hfq"  # 后复权！不要改 qfq
MIN_TRADING_DAYS = 250
SLEEP = 0.3          # 请求间隔（秒），腾讯源限制宽松
MAX_RETRIES = 3
# =====================
...
codes = stocks['code'].str[2:].tolist()  # "sh600519" → "600519"
...
# 验证：后复权价格不应有负数
neg = combined[combined['收盘'] < 0]
if len(neg) > 0:
    print(f"⚠️ 警告：{len(neg)} 行负价格，{neg['股票代码'].nunique()} 只股票（不应出现——可能用了 qfq）")

# 腾讯源无涨跌幅/换手率字段，需要时可用 pandas 计算：
# combined['涨跌幅'] = combined.groupby('股票代码')['收盘'].pct_change()
