#!/usr/bin/env python3
"""因子计算模块 — 从 build_factors_pl 抽取，供 update_daily 等复用"""
import polars as pl

def calc_factors(df):
    """输入原始日K（日期/开盘/收盘/最高/最低/成交量/turnover/成交额/股票代码），输出45因子"""
    ...
    for n in [5, 10, 20]:
        df = df.with_columns(
            pl.col('ret_1d').rolling_sum(n, min_samples=n).over('股票代码').alias(f'ret_{n}d'))
    df = df.with_columns([
    ...
    df = df.with_columns([
        up_flag.cum_sum().over(['股票代码', up_flag.rle_id().over('股票代码')]).alias('up_streak'),
        dn_flag.cum_sum().over(['股票代码', dn_flag.rle_id().over('股票代码')]).alias('down_streak'),
    ])
    return df.drop(['_tr'])
